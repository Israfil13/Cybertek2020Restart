package Num236_BookShelf;

import day46.CheckingAccount;
...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
        ArrayList<String> articles=new ArrayList<>(Arrays.asList("a","an","the"));
        ArrayList<String> conjuctions=new ArrayList<>(Arrays.asList("and", "but", "for", "nor", "or", "so", "yet"));
        ArrayList<String> prepositions=new ArrayList<>(Arrays.asList("at", "by", "in", "into", "near", "of", "on", "to", "than", "via"));

        for (int i = 0; i <articles.size() ; i++) {
            if (word.toLowerCase().equals(articles.get(i)))
                return false;
        }
        for (int i = 0; i <conjuctions.size() ; i++) {
            if (word.toLowerCase().equals(conjuctions.get(i)))
                return false;
        }
        for (int i = 0; i <prepositions.size() ; i++) {
            if (word.toLowerCase().equals(prepositions.get(i)))
                return false;
        }
        ...
        if(title.length()==1) {
            ...
            for ( int x = 1 ; x < arr.length ; x++) {
                if(isTitleWord(arr[x]) ){
                    result+=capitalize(arr[x])+" ";
                } else {
                    result+=arr[x].toLowerCase()+" ";
                }
            }
